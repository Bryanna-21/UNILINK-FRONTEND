...
const Courses = () => {
  ...
  const fetchCourses = async () => {
    setLoading(true);
    try { const raw=await courseService.getMyCourses(); const list=Array.isArray(raw)?raw:[]; setCourses(list); setFilteredCourses(list); }
    catch(error){ setCourses([]); setFilteredCourses([]); setToast({show:true,type:"error",message:error.response?.data?.message||"Could not load your courses. Please try again."}); }
